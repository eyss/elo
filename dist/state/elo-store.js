...
export class EloStore {
    constructor(eloService, profilesStore) {
        ...
        this.eloService.cellClient.addSignalHandler(signal => {
            if (signal.data.payload.type === 'NewGameResult') {
                this.handleNewGameResult(signal.data.payload.game_result, signal.data.payload.entry_hash);
            }
        });
    }
    get myAgentPubKey() {
    ...
    createEloRankingStore(chunkSize) {
    ...
    async handleNewGameResult(gameResult, gameResultHash) {
        const players = [
